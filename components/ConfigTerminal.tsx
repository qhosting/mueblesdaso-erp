...
`;
  ...
  const ROADMAP_DATA = [
    ...
    { cat: 'CRM Admin', item: 'Antigüedad y Pronóstico Financiero', status: 'done' },
  ...
  return (
    <div className="p-6 h-full overflow-y-auto bg-slate-50">
      ...
      <div className="flex space-x-2 mb-6 border-b border-slate-200 overflow-x-auto scrollbar-none">
        {[
          { id: 'roadmap', label: 'Hoja de Ruta', icon: <ListChecks className="w-4 h-4" /> },
          { id: 'prod', label: 'Verificación Prod', icon: <ShieldCheck className="w-4 h-4" /> },
          { id: 'docker', label: 'Docker', icon: <Layers className="w-4 h-4" /> },
          { id: 'sql', label: 'PostgreSQL', icon: <Database className="w-4 h-4" /> },
          { id: 'waha', label: 'API WAHA', icon: <Smartphone className="w-4 h-4" /> },
        ].map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id as any)}
            className={`flex items-center gap-2 px-6 py-3 text-[10px] font-black uppercase tracking-widest transition-all shrink-0 ${activeTab === tab.id ? 'border-b-2 border-blue-600 text-blue-600 bg-white rounded-t-xl' : 'text-slate-500 hover:text-slate-700'
              }`}
          >
            {tab.icon}
            {tab.label}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {activeTab === 'prod' && (
            <div className="bg-white border border-slate-200 rounded-[2.5rem] p-8 shadow-sm space-y-8 animate-in fade-in zoom-in-95 duration-300">
              ...
              <div className="bg-slate-900 text-white p-6 rounded-3xl border border-slate-800">
                ...
                <p className="text-sm font-medium text-slate-300">Sincroniza el repositorio de GitHub con Easypanel y añade los recursos de PostgreSQL y WAHA desde la tienda de aplicaciones.</p>
              </div>
            </div>
          )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-slate-200 p-8 rounded-[2.5rem] shadow-sm">
            <h4 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-4">Notas de PostgreSQL</h4>
